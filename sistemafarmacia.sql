...
-- Selecionando com WEHE e AND
SELECT * FROM farmacia
WHERE cidade = 'são paulo' AND UF ='SP';

-- Selecionando com WEHE e OR
SELECT * FROM farmacia
WHERE cidade = 'são paulo' OR UF ='Campinas';

SELECT telefone 
FROM farmacia
WHERE cidade = 'são paulo' OR UF ='Campinas';

-- Selecione com BETWEEN (entre) e AND
SELECT *FROM produto
WHERE valorproduto BETWEEN 10.00 AND 30.00;

-- Deletendo um dado de uma tabela
DELETE FROM produto
WHERE codproduto = 4;
...
ON farmacianova.*
TO 'usuario_faramacia'@'localhost';
...
REVOKE INSERT
ON farmacianova.*
FROM 'usuario_faramacia'@'localhost';

--Revogar todas  permissões de usuário
